...
~~As of version 1.2.3 the Grafana dashboard includes an "Update" button.~~ As of version 1.4.0 this button has been moved to the admin panel on the website. Pressing this button is meant to run `scripts/update.sh` in production. For this, you need to run `cargo run` in `script_service` in a `screen` or `tmux` session which will listen to requests from Grafana for this. This is not essential to run the bot in Docker and is only for that button. 
