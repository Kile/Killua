...
class Buttons(View):
    ...
    async def _get_embed(self) -> None:
        ...
        if self.func:
            self.embed = (
                (self.func(self.page, self.embed, self.pages))
                if not iscoroutinefunction(self.func)
                else (await self.func(self.page, self.embed, self.pages))
            )
        else:
    ...
    async def _handle_file(self, interaction: discord.Interaction) -> None:
        ...
        await self.paginator.__class__(
            **vars(self.paginator)
        ).start()  # purpose of this is that this calls the subclasses `start` if a subclass exists
    ...
    async def _edit_message(self, interaction: discord.Interaction) -> None:
        ...
        if self.defer:
            await interaction.response.edit_message(
                content="processing...", embed=interaction.message.embeds[0], view=None
            )
...
class Paginator:
    ...
    async def _start(self) -> Buttons:
        ...
        if (
            not self.view.ignore
        ):  # This is False when the message has been deleted/should not get their buttons disabled
