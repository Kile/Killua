...
class Patreon:
    ...
    def _catch(self, d: dict) -> Union[str, None]:
        ...
        try:
            ...
            KeyError
        ):  # In case this happens I still want to have a value, just `None`
            return None
    ...
    def _get_user_info(self, data: list, user: str) -> dict:
        ...
        return next(
            (
                x
                for x in data
                if "user" in x["relationships"].keys()
                and x["relationships"]["user"]["data"]["id"] == user
            ),
            None,
        ...
        for i in data["included"]:
            if i["type"] == "user":
                ...
                try:
                    res.append(
                        {
                            "discord": self._int_else(self._catch(i)),
                            "tier": sorted(
                                user["relationships"]["currently_entitled_tiers"][
                                    "data"
                                ],
                    ...
                    Exception
                ):  # If this happens something with the tier went wrong. This means they are no longer subscribed and I want to ignore that case
                    pass
...
class Premium(commands.Cog):
    ...
    def _get_differences(self, current: Patrons, saved: List[dict]) -> List[dict]:
        ...
        new_patrons = [
            *[
                x
                for x in current.patrons
                if x["discord"] not in [x["id"] for x in saved]
            ],
            *[
                {"discord": b, "tier": list(PATREON_TIERS.keys())[0]}
                for b in boosters
                if b not in [s["id"] for s in saved]
            ],
        ]
        # Kinda hacky ways to do it but saves lines
        removed_patrons = [
            {"discord": x["id"], "tier": None}
            for x in saved
            if x["id"] not in current and (x["id"] not in boosters and not self.invalid)
        ]
        different_badges = [
            x
            for x in current.patrons
            if x["tier"] not in [y["id"] for y in saved if y["id"] == x["discord"]]
        ]
    ...
    def _get_subscription_entitlements(
        ...
        for sku in self.client.cached_skus:
            if sku.type != discord.SKUType.subscription:
                continue  # ignore non-subscription SKUs
            if sku.flags.guild_subscription:
                continue  # ignore guild subscriptions
            entitlements.extend(
                [
                    entitlement
                    for entitlement in all
                    if entitlement.sku_id == sku.id and not entitlement.is_expired()
                ]
            )
    ...
    async def on_entitlement_create(self, ent: discord.Entitlement):
        ...
        # DM user
        try:
            user = self.client.get_user(ent.user_id) or await self.client.fetch_user(
                ent.user_id
            )
            await user.send(
                f"Thank you for supporting me! You have received {amount}x {LOOTBOXES[lootbox]['name']}{LOOTBOXES[lootbox]['emoji']}! Open it with `k!open`"
            )
    ...
    async def on_entitlement_update(self, ent: discord.Entitlement):
        ...
        index = next(
            (
                i
                for i, e in enumerate(self.client.cached_entitlements)
                if e.id == ent.id
            ),
            None,
        )
    ...
    @premium_user_only()
    @premium.command(
        extras={"category": Category.OTHER, "id": 67}, usage="guild <add/remove>"
    )
    @discord.app_commands.describe(
        action="Wether to add or remove the premium status of the current server"
    )
    ...
    async def weekly(self, ctx: commands.Context):
        ...
        if user.weekly_cooldown and user.weekly_cooldown > datetime.now():
            ...
            return await ctx.send(
                f"You can claim your weekly lootbox the next time {cooldown}"
            )
    ...
    @check()
    @premium.command(
        aliases=["support"], extras={"category": Category.FUN, "id": 69}, usage="info"
    )
    async def info(self, ctx: commands.Context):
        """List of all the premium perks that come along with being a patron!"""
        view = discord.ui.View()
        view.add_item(
            discord.ui.Button(label="Get premium", url="https://patreon.com/kilealkuri")
        )
