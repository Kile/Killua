...
'https://dzt1km7tv28ex.cloudfront.net/u/466410804707590144_35s_d.jpg']
...
class smallcommands(commands.Cog):
    ...
    async def pat(self, ctx, members: commands.Greedy[discord.Member]=None):
        if members:
            ...
            embed = discord.Embed.from_dict({
                'title': random.choice(pattext).replace('(a)', ctx.author.name).replace('(u)', memberlist),
                'image':{
                    'url': random.choice(patgif)},

                'color': 0x1400ff
                })
            await ctx.send(embed=embed)
        else:
            await ctx.send('You provided no one to pat.. Should- I pat you?')
            def check(m):
                return m.content.lower() == 'yes' and m.author == ctx.author

            msg = await self.client.wait_for('message', check=check, timeout=60) 
            embed = discord.Embed.from_dict({
                'title': random.choice(pattext).replace('(a)', 'Killua').replace('(u)',ctx.author.name),
                'image':{
                    'url': random.choice(patgif)
                },
                'color': 0x1400ff
