  ...
  async def ban(self, ctx, member: discord.Member, *,reason=None):
    ...
    if ctx.channel.permissions_for(ctx.author).ban_members == True:
        ...
        try:
          ...
          pass
        await member.ban(reason=reason or "No reason provided", delete_message_days=1)
...
async def mute(self, ctx, member: discord.Member, timem=None, *,reason=None):
    #h Mutes the user specified for either x or an unlimited amount of minutes. Be aware though that if the bot restarts the countdown of minutes is reset, so the user is not unmuted
    ...
    if ctx.channel.permissions_for(ctx.author).manage_roles == True:
        ...
        if timem:
            ...
            if timem.isdigit() is True:
                ...
                if timem.lower() == 'unlimited' or timem.lower() == 'standart' ot timem.lower() == 'u':
                    string = True
